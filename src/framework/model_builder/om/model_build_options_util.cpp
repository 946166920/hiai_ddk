...
namespace hiai {
static HIAI_MR_DynamicShapeConfig* ConvertCDynamicShapeConfig(const DynamicShapeConfig& config)
{
    ...
    HIAI_MR_DynamicShapeConfig_SetMaxCacheNum(dynamicShapeConfig, config.maxCachedNum);
    HIAI_MR_DynamicShapeConfig_SetCacheMode(
        dynamicShapeConfig, static_cast<HIAI_DYNAMIC_SHAPE_CACHE_MODE>(config.cacheMode));
    return dynamicShapeConfig;
}

static HIAI_MR_OpDeviceOrder* ConvertToCOpDeviceConfig(
    const std::string& opName, const std::vector<ExecuteDevice>& deviceList)
{
    HIAI_MR_OpDeviceOrder* opDeviceOrder = HIAI_MR_OpDeviceOrder_Create();
    HIAI_EXPECT_NOT_NULL_R(opDeviceOrder, nullptr);

    HIAI_MR_OpDeviceOrder_SetOpName(opDeviceOrder, opName.c_str());
    ...
    HIAI_MR_OpDeviceOrder_SetDeviceOrder(opDeviceOrder, deviceList.size(), cDeviceList);
    return opDeviceOrder;

FREE_ALL:
    HIAI_MR_OpDeviceOrder_Destroy(&opDeviceOrder);
    return nullptr;
}

static HIAI_MR_OpDeviceOrder** ConvertToCOpDeviceConfigList(
    const std::map<std::string, std::vector<ExecuteDevice>>& opDeviceConfigs)
{
    size_t size = sizeof(HIAI_MR_OpDeviceOrder*) * opDeviceConfigs.size();
    HIAI_MR_OpDeviceOrder** opDeviceOrders = (HIAI_MR_OpDeviceOrder**)malloc(size);
...
FREE_ALL:
    for (i = 0; i < opDeviceConfigs.size(); i++) {
        if (opDeviceOrders[i] != nullptr) {
            HIAI_MR_OpDeviceOrder_Destroy(&opDeviceOrders[i]);
        }
    }
    free(opDeviceOrders);
...
static HIAI_MR_CLCustomization* ConvertToCclCustomization(const std::string& opName, const std::string& customization)
{
    HIAI_MR_CLCustomization* cLCustomization = HIAI_MR_CLCustomization_Create();
    HIAI_EXPECT_NOT_NULL_R(cLCustomization, nullptr);

    HIAI_MR_CLCustomization_SetOpName(cLCustomization, opName.c_str());
    HIAI_MR_CLCustomization_SetCustomization(cLCustomization, customization.c_str());
    return cLCustomization;
}

static HIAI_MR_CLCustomization** ConvertToCclCustomizationList(
    const std::map<std::string, std::string>& clCustomizations)
{
    size_t size = sizeof(HIAI_MR_CLCustomization*) * clCustomizations.size();
    HIAI_MR_CLCustomization** cLCustomization = (HIAI_MR_CLCustomization**)malloc(size);
    HIAI_EXPECT_NOT_NULL_R(cLCustomization, nullptr);
...
FREE_ALL:
    for (i = 0; i < clCustomizations.size(); i++) {
        if (cLCustomization[i] != nullptr) {
            HIAI_MR_CLCustomization_Destroy(&cLCustomization[i]);
        }
    }
...
static HIAI_MR_ModelDeviceConfig* ConvertCModelDeviceConfig(const ModelDeviceConfig& config)
{
    HIAI_MR_ModelDeviceConfig* modelDeviceConfig = HIAI_MR_ModelDeviceConfig_Create();
    HIAI_EXPECT_NOT_NULL_R(modelDeviceConfig, nullptr);

    HIAI_MR_ModelDeviceConfig_SetDeviceConfigMode(
        modelDeviceConfig, static_cast<HIAI_DEVICE_CONFIG_MODE>(config.deviceConfigMode));
    HIAI_MR_ModelDeviceConfig_SetFallBackMode(modelDeviceConfig, static_cast<HIAI_FALLBACK_MODE>(config.fallBackMode));
    if (config.deviceConfigMode == DeviceConfigMode::MODEL_LEVEL) {
        ...
        if (cDeviceTypeList == nullptr) {
            HIAI_MR_ModelDeviceConfig_Destroy(&modelDeviceConfig);
            return nullptr;
        }
        HIAI_MR_ModelDeviceConfig_SetModelDeviceOrder(
            modelDeviceConfig, config.modelDeviceOrder.size(), cDeviceTypeList);
    } else if (config.deviceConfigMode == DeviceConfigMode::OP_LEVEL) {
        if (!config.opDeviceOrder.empty()) {
            HIAI_MR_OpDeviceOrder** cOpDeviceConfigList = ConvertToCOpDeviceConfigList(config.opDeviceOrder);
            if (cOpDeviceConfigList == nullptr) {
                HIAI_MR_ModelDeviceConfig_Destroy(&modelDeviceConfig);
                return nullptr;
            }
            HIAI_MR_ModelDeviceConfig_SetOpDeviceOrder(
                modelDeviceConfig, config.opDeviceOrder.size(), cOpDeviceConfigList);
        }
        HIAI_MR_ModelDeviceConfig_SetDeviceMemoryReusePlan(
...
HIAI_MR_ModelBuildOptions* ModelBuildOptionsUtil::ConvertToCBuildOptions(const ModelBuildOptions& options)
{
    HIAI_MR_ModelBuildOptions* buildOptions = HIAI_MR_ModelBuildOptions_Create();
    HIAI_EXPECT_NOT_NULL_R(buildOptions, nullptr);

    HIAI_MR_DynamicShapeConfig* cDynamicShapeConfig = nullptr;
    HIAI_MR_ModelDeviceConfig* cModelDeviceConfig = nullptr;
    if (!options.inputTensorDescs.empty()) {
        ...
        HIAI_MR_ModelBuildOptions_SetInputTensorDescs(buildOptions, options.inputTensorDescs.size(), inputDescs);
    }

    HIAI_MR_ModelBuildOptions_SetFormatModeOption(
        buildOptions, static_cast<HIAI_FORMAT_MODE_OPTION>(options.formatMode));

    HIAI_MR_ModelBuildOptions_SetPrecisionModeOption(
    ...
    HIAI_MR_ModelBuildOptions_SetDynamicShapeConfig(buildOptions, cDynamicShapeConfig);

    cModelDeviceConfig = ConvertCModelDeviceConfig(options.modelDeviceConfig);
    if (cModelDeviceConfig == nullptr) {
        goto FREE_ALL;
    }
    HIAI_MR_ModelBuildOptions_SetModelDeviceConfig(buildOptions, cModelDeviceConfig);

    HIAI_MR_ModelBuildOptions_SetTuningStrategy(
        buildOptions, static_cast<HIAI_TUNING_STRATEGY>(options.tuningStrategy));

    (void)HIAI_MR_ModelBuildOptions_SetEstimatedOutputSize(buildOptions, options.estimatedOutputSize);
    if (options.quantizeConfig != "") {
        (void)HIAI_MR_ModelBuildOptions_SetQuantizeConfig(buildOptions,
