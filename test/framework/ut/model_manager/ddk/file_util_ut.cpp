...
/*
* 测试用例名称: model_manager_file_util_001
* 测试用例描述： 打开一个文件
* 预置条件 : 不存在的文件
* 操作步骤: 1.读取文件
* 预期结果 : 读取文件失败，返回空指针
...
TEST_F(FileUtilUt, model_manager_file_util_001)
{
    string fileName = "bin/llt/framework/domi/modelmanager/test.om";
    string mode = "w+";
    FileUtil fileUtil;
    auto ret = fileUtil.OpenFile(fileName, mode);
    EXPECT_TRUE(ret == nullptr);
}

/*
* 测试用例名称: model_manager_file_util_002
* 测试用例描述： 打开一个文件
* 预置条件 : 空文件名
* 操作步骤: 1.读取文件
* 预期结果 : 读取文件失败，返回空指针
...
TEST_F(FileUtilUt, model_manager_file_util_002)
{
    string fileName = "";
    string mode = "w+";
    FileUtil fileUtil;
...
/*
* 测试用例名称: model_manager_file_util_006
* 测试用例描述： 关闭文件
* 预置条件 :
* 操作步骤:
* 预期结果 : 成功
...
TEST_F(FileUtilUt, model_manager_file_util_006)
{
    const uint32_t defaultSize = 200; // default allocated buffer size, 200 MB
    const uint64_t megaSize = 1024;   // MB size
    const string modelPath = "out/testbuffer.om";
    ...
    FileUtil fileUtil;
    string mode = "w+";
    FILE *fp = fileUtil.OpenFile(modelPath, mode);
...
/*
* 测试用例名称: model_manager_file_util_008
* 测试用例描述： 获取文件大小
* 预置条件 :
* 操作步骤:
* 预期结果 : 成功
...
TEST_F(FileUtilUt, model_manager_file_util_008)
{
    const uint32_t defaultSize = 200; // default allocated buffer size, 200 MB
    const uint64_t megaSize = 1024;   // MB size
    const string modelPath = "out/testbuffer.om";
